...
parser = argparse.ArgumentParser(description="Example of independent low-rank matrix axalysis (ILRMA)")
    ...
    for title in titles:
        for degree in degrees:
            for mic_idx in mic_indices:
                convolve_mird(data_root, title, reverb=reverb, degree=degree, mic_intervals=mic_intervals, mic_idx=mic_idx, sr=target_sr, samples=samples)


def convolve_mird(data_root, title, reverb=0.160, degree=0, mic_intervals="3-3-3-8-3-3-3", mic_idx=0, sr=16000, samples=None):
    rir_path = os.path.join(data_root, "MIRD/Reverb{:.3f}_{}/Impulse_response_Acoustic_Lab_Bar-Ilan_University_(Reverberation_{:.3f}s)_{}_1m_{:03d}.mat".format(reverb, mic_intervals, reverb, mic_intervals, degree))
