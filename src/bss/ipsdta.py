...
class GaussIPSDTA(IPSDTAbase):
    ...
    def update_basis_em(self):
        ...
        def _update_basis_em(estimation, basis, activation):
            """
            Args:
                estimation: (n_sources, n_frames, n_blocks, n_neighbors)
                basis: (n_sources, n_blocks, n_neighbors, n_neighbors, n_basis)
                activation: (n_sources, n_basis, n_frames)
            """
            _, n_blocks, n_neighbors, _, _ = basis.shape

            U, V = basis.transpose(0, 4, 1, 2, 3), activation # (n_sources, n_basis, n_blocks, n_neighbors, n_neighbors), (n_sources, n_basis, n_frames)
            Y = estimation.reshape(n_sources, n_frames, n_blocks, n_neighbors, 1)
            ...
            V[V < eps] = eps
            U = np.mean(Phi[:, :, :, :, :, :] / V[:, :, :, np.newaxis, np.newaxis, np.newaxis], axis=2) # (n_sources, n_basis, n_blocks - n_remains, n_neighbors, n_neighbors)
            U = to_PSD(U, axis1=3, axis2=4, eps=eps)
            basis = U.transpose(0, 2, 3, 4, 1)
            activation = V
        ...
        if n_remains > 0:
            ...
            U_low, V = _update_basis_em(Y_low, basis=U_low, activation=V)
            U_high, V = _update_basis_em(Y_high, basis=U_high, activation=V)

            U = U_low, U_high
        else:
            Y = Y.reshape(n_sources, n_frames, n_blocks, n_neighbors)

            U, V = _update_basis_em(Y, basis=U, activation=V)
    ...
    def update_activation_em(self):
        ...
        def _update_activation_factor(estimation, basis, activation):
            ...
            U, V = basis.transpose(0, 4, 1, 2, 3), activation # (n_sources, n_basis, n_blocks, n_neighbors, n_neighbors), (n_sources, n_basis, n_frames)
            Y = estimation.reshape(n_sources, n_frames, n_blocks, n_neighbors, 1) # (n_sources, n_frames, n_blocks * n_neighbors)
    ...
    def update_basis_mm(self):
        ...
        def _update_basis_mm(estimation, basis, activation):
            """
            Args:
                estimation: (n_sources, n_frames, n_blocks, n_neighbors)
                basis: (n_sources, n_blocks, n_neighbors, n_neighbors, n_basis)
            ...
            R_basis = U[:, :, np.newaxis, :, :, :] * V[:, :, :, np.newaxis, np.newaxis, np.newaxis] # (n_sources, n_basis, n_frames, n_blocks - n_remains, n_neighbors, n_neighbors)
            R = np.sum(R_basis, axis=1) # (n_sources, n_frames, n_blocks - n_remains, n_neighbors, n_neighbors), (n_sources, n_frames, n_remains, n_neighbors + 1, n_neighbors + 1)
            R = to_PSD(R, axis1=3, axis2=4, eps=eps)
            inv_R = np.linalg.inv(R)
            inv_R = to_PSD(inv_R, axis1=3, axis2=4, eps=eps)
            
            yy = y @ y.transpose(0, 1, 2, 4, 3).conj() + eps * np.eye(n_neighbors)
            RyyR = inv_R @ yy @ inv_R # (n_sources, n_frames, n_blocks - n_remains, n_neighbors, n_neighbors)
            S = np.sum(V[:, :, :, np.newaxis, np.newaxis, np.newaxis] * RyyR[:, np.newaxis, :, :, :, :], axis=2) # (n_sources, n_basis, n_blocks - n_remains, n_neighbors, n_neighbors)
            T = np.sum(V[:, :, :, np.newaxis, np.newaxis, np.newaxis] * inv_R[:, np.newaxis, :, :, :, :], axis=2) # (n_sources, n_basis, n_blocks - n_remains, n_neighbors, n_neighbors)
        ...
        if n_remains > 0:
            ...
            U_low, V = _update_basis_mm(y_low, basis=U_low, activation=V)
            U_high, V = _update_basis_mm(y_high, basis=U_high, activation=V)

            U = U_low, U_high
        else:
            U, V = _update_basis_mm(Y, basis=U, activation=V)
    ...
    def update_activation_mm(self):
        ...
        def _update_activation_factor(estimation, basis, activation):
            ...
            basis = U.transpose(0, 2, 3, 4, 1) # (n_sources, n_blocks, n_neighbors, n_neighbors, n_basis)
            activation = V

            return numerator, denominator, basis, activation
    ...
    def update_spatial_model_fixed_point(self):
        ...
        def _update_spatial_model_fixed_point(input, demix_filter, basis, activation, fixed_point):
            """
            Args:
                input: (n_channels, n_frames, n_blocks, n_neighbors)
                demix_filter: ()
                basis: (n_sources, n_blocks, n_neighbors, n_neighbors, n_basis)
                activation: (n_sources, n_basis, n_frames)
                fixed_point: (n_sources, n_blocks * n_neighbors)
            """
            n_channels, n_frames, n_blocks, n_neighbors = input.shape

            W_Hermite = demix_filter
            U, V = basis, activation

            U = basis.transpose(0, 4, 1, 2, 3) # (n_sources, n_basis, n_blocks, n_neighbors, n_neighbors)
            R = np.sum(U[:, :, np.newaxis, :, :, :] * V[:, :, :, np.newaxis, np.newaxis, np.newaxis], axis=1) # (n_sources, n_frames, n_blocks, n_neighbors, n_neighbors)
            R = to_PSD(R, axis1=3, axis2=4)
            inv_R = np.linalg.inv(R.conj()) # (n_sources, n_frames, n_blocks, n_neighbors, n_neighbors)

            X = input.transpose(1, 2, 3, 0).reshape(n_frames, n_blocks, n_neighbors * n_channels)
            ...
            A = np.linalg.inv(W_Hermite) # (n_blocks * n_neighbors, n_channels, n_sources)
            A = A.transpose(2, 0, 1) # (n_sources, n_blocks * n_neighbors, n_channels)
            ...
            # Update Lambda
            L = fixed_point.reshape(n_sources, n_blocks, n_neighbors, 1)
            denominator = B.swapaxes(2, 3) @ L.conj()
            denominator = denominator.squeeze(axis=3) # (n_sources, n_blocks, n_neighbors)
            denominator[np.abs(denominator) < eps] = eps
            L = 1 / denominator # (n_sources, n_blocks, n_neighbors)

            inv_G = inv_G.reshape(n_sources, n_blocks, n_neighbors, n_channels, n_neighbors, n_channels)
            GL = inv_G * L[:, :, np.newaxis, np.newaxis, :, np.newaxis] # (n_sources, n_blocks, n_neighbors, n_channels, n_neighbors, n_channels)
            GL = GL.reshape(n_sources, n_blocks, n_neighbors * n_channels, n_neighbors * n_channels)

            fixed_point = L.reshape(n_sources, n_blocks * n_neighbors)
            ...
            W = W.transpose(1, 0, 2) # (n_blocks * n_neighbors, n_sources, n_channels)
            demix_filter = W.conj() # (n_blocks * n_neighbors, n_sources, n_channels)

            return demix_filter, fixed_point
        ...
        U, V = self.basis, self.activation # _, (n_sources, n_basis, n_frames)
        L = self.fixed_point # (n_sources, n_bins)
        ...
        if n_remains > 0:
            ...
            W_Hermite_low, W_Hermite_high = np.split(W_Hermite, [(n_blocks - n_remains) * n_neighbors], axis=0) # ((n_blocks - n_remains) * n_neighbors, n_sources, n_channels), (n_remains * (n_neighbors + 1), n_sources, n_channels)

            U_low, U_high = U
            L_low, L_high = np.split(L, [(n_blocks - n_remains) * n_neighbors], axis=1)

            W_Hermite_low, L_low = _update_spatial_model_fixed_point(X_low, W_Hermite_low, basis=U_low, activation=V, fixed_point=L_low)
            W_Hermite_high, L_high = _update_spatial_model_fixed_point(X_high, W_Hermite_high, basis=U_high, activation=V, fixed_point=L_high)

            W_Hermite = np.concatenate([W_Hermite_low, W_Hermite_high], axis=0)
            L = np.concatenate([L_low, L_high], axis=1) # (n_sources, n_bins)
        else:
            X = X.reshape(n_channels, n_frames, n_blocks, n_neighbors)
            W_Hermite, L = _update_spatial_model_fixed_point(X, W_Hermite, basis=U, activation=V, fixed_point=L)
    ...
    def update_spatial_model_vcd(self):
        ...
        def _update_spatial_model_vcd_neighbor(input, demix_filter, inv_psdtf, covariance_input=None, source_idx=0, neighbor_idx=0):
            """
            demix_filter: (n_blocks, n_neighbors, n_sources, n_channels)
            inv_psdtf: (n_blocks, n_neighbors, n_neighbors', n_frames)

            """
            n_blocks, n_neighbors, _, _ = inv_psdtf.shape
            ...
            e_n = np.eye(n_sources)[source_idx] # (n_blocks, n_channels)
            e_n = np.tile(e_n, reps=(n_blocks, 1))
            ...
            XXw_n = X[:, neighbor_idx, np.newaxis, :, :] * Xw_n[:, :, :, np.newaxis] # (n_blocks - n_remains, n_neighbors', n_frames, n_channels)
            RXXw_n = np.mean(inv_R_n[:, neighbor_idx, :, :, np.newaxis] * XXw_n, axis=2) # (n_blocks - n_remains, n_neighbors', n_channels)
            gamma_in = np.sum(mask[np.newaxis, neighbor_idx, :, np.newaxis] * RXXw_n, axis=1) # (n_blocks - n_remains, n_channels)

            WQ_in = W[:, neighbor_idx, :, :] @ Q_in # (n_blocks - n_remains, n_sources, n_channels)
            zeta_in = np.linalg.solve(WQ_in, e_n) # (n_blocks - n_remains, n_channels)
            zeta_hat_in = np.linalg.solve(Q_in, gamma_in) # (n_blocks - n_remains, n_channels)
            eta_in = np.squeeze(zeta_in[:, np.newaxis, :].conj() @ Q_in @ zeta_in[:, :, np.newaxis], axis=(1, 2)) # (n_blocks - n_remains,)
            eta_hat_in = np.squeeze(zeta_in[:, np.newaxis, :].conj() @ Q_in @ zeta_hat_in[:, :, np.newaxis], axis=(1, 2)) # (n_blocks - n_remains,)
        ...
        if n_remains > 0:
            ...
            for source_idx in range(n_sources):
                U_n_low, U_n_high = U_low[source_idx], U_high[source_idx] # (n_basis, n_blocks - n_remains, n_neighbors', n_neighbors), (n_basis, n_remains, n_neighbors' + 1, n_neighbors + 1)
                ...
                for neighbor_idx in range(n_neighbors):
                    W_low = _update_spatial_model_vcd_neighbor(X_low, W_low, inv_R_n_low, covariance_input=XX_low, source_idx=source_idx, neighbor_idx=neighbor_idx)
                    W_high = _update_spatial_model_vcd_neighbor(X_high, W_high, inv_R_n_high, covariance_input=XX_high, source_idx=source_idx, neighbor_idx=neighbor_idx)
                
                W_high = _update_spatial_model_vcd_neighbor(X_high, W_high, inv_R_n_high, covariance_input=XX_high, source_idx=source_idx, neighbor_idx=n_neighbors)
            ...
            for source_idx in range(n_sources):
                ...
                for neighbor_idx in range(n_neighbors):
                    W = _update_spatial_model_vcd_neighbor(X, W, inv_R_n, covariance_input=XX, source_idx=source_idx, neighbor_idx=neighbor_idx)
