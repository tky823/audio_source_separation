        ...
        if n_remains > 0:
            ...
            for source_idx in range(n_sources):
                ...
                for neighbor_idx in range(n_neighbors):
                    ...
                    w_in_low = np.where(condition_low[:, np.newaxis] and np.logical_not(condition_if_low), weight_low[:, np.newaxis] * zeta_in_low - zeta_hat_in_low, w_in_low)
                ...
                for neighbor_idx in range(n_neighbors + 1):
                    ...
                    w_in_high = np.where(condition_high[:, np.newaxis] and np.logical_not(condition_if_high), weight_high[:, np.newaxis] * zeta_in_high - zeta_hat_in_high, w_in_high)
            ...
            for source_idx in range(n_sources):
                ...
                for neighbor_idx in range(n_neighbors):
                    ...
                    w_in = np.where(condition[:, np.newaxis] and np.logical_not(condition_if), weight[:, np.newaxis] * zeta_in - zeta_hat_in, w_in)
